...
``edm workspace use`` writes the per-terminal *and* per-session pointers (never a
...
# git-visible and travels with the umbrella.                                       #
# --------------------------------------------------------------------------- #
_LIVE_BEGIN = "# >>> edm live workspaces (managed by `edm workspace live`) >>>"
_LIVE_END = "# <<< edm live workspaces <<<"
...
_WS_MEM_HEADER = (
    ...
    "`edm workspace remember \"…\"`, search with `edm workspace recall [--query …]`, drop "
    "one with `edm workspace forget <slug>`. Never put secrets here (vault only).\n"
)
...
_VISION_PLACEHOLDER = (
    ...
    'Set it with `edm workspace vision "…"` (or edit this file)._'
)

_CHARTER_TEMPLATE = """# {name}

> **Living charter** for this workspace — its north star and shared context.
> Keep it current as the work evolves (edit this file, or `edm workspace vision "…"`).
...
(append with `edm workspace note "…"`).
"""
...
# by an older umbrella can lack files a newer one expects (workspace.md, memory/, #
...
# `edm workspace reinit` heals it. To ship a new structural element in future:    #
