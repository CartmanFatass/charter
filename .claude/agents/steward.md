...
- **You may delegate to these personas: `statusline`, `release`, `forge`.** Hand them a sub-task (Agent tool, `subagent_type: <name>`). You do NOT hold their credentials and their tools are not auto-approved for you — that is what `borrows:` is for, and it is deliberate: doing their work yourself should cost more than handing it over.
